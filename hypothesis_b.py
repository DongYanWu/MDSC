...
for current_column, test_results in results.items():
	...
	# For most of the current columns (y01, y02, y03, y04, y05, y06, y08, y09, y10), both the t-test and K-S test strongly reject the null hypothesis, indicating that the testing data does not come from condition b.
	# •	y07:
	# •	The t-test does not reject the null hypothesis, meaning that the testing data might come from condition b based on a comparison of means.
	# •	However, the K-S test suggests that the distributions are significantly different for y07, meaning that the testing data likely does not come from condition b based on distribution.